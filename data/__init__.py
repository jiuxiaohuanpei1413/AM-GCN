...
def get_transform(args, is_train=True):
    if is_train:
        transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.4554101, 0.47250092, 0.47067854], std=[0.25613797, 0.25841394, 0.286263])
            ])
    else:
        transform = transforms.Compose([
            transforms.Resize((args.image_size,args.image_size)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.46282277, 0.48090994, 0.47384837], std=[0.2611891, 0.2611104, 0.2886833])
        ])
    return transform
